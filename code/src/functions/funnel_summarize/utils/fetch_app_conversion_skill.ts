...
export class FetchConversionAppVersionSkill implements Skill {
  
  // devrevSDK is used to make API calls using service account token.
  devrevSDK: SdkUtils;
  // Name of the skill.
  Name = 'FetchConversionAppVersionSkill';
  // Description of the skill.
  Description = 'Fetches the funnel app conversion based on funnel id';
  ...
  async Execute(skillCall: SkillCall): Promise<any> {
    ...
    return this.devrevSDK.getConversionAppVersionSkill(skillCall.args.id, skillCall.args.appId, skillCall.args.appVersion, skillCall.args.platform, skillCall.args.fromDate, skillCall.args.toDate);
  }
}
