...
export class GetFunnelDropoutSkill implements Skill {
  
  // devrevSDK is used to make API calls using service account token.
  devrevSDK: SdkUtils;
  // Name of the skill.
  Name = 'GetFunnelDropoutSkill';
  // Description of the skill.
  Description = 'Fetches the funnel dropout based on funnel id';
  ...
  async Execute(skillCall: SkillCall): Promise<any> {
    ...
    return this.devrevSDK.getFunnelDropoutSkill(skillCall.args.id, skillCall.args.appId, skillCall.args.appVersion, skillCall.args.platform, skillCall.args.fromDate, skillCall.args.toDate);
  }
}
