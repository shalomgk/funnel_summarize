...
export class FetchFunnelAPIOverviewPartOneSkill implements Skill {
  
  // devrevSDK is used to make API calls using service account token.
  devrevSDK: SdkUtils;
  // Name of the skill.
  Name = 'FetchFunnelAPIOverviewPartOneSkill';
  ...
  async Execute(skillCall: SkillCall): Promise<any> {
    ...
    return this.devrevSDK.getFunnelOverviewPartOne(skillCall.args.id, skillCall.args.appId, skillCall.args.appVersion, skillCall.args.platform, skillCall.args.fromDate, skillCall.args.toDate, skillCall.args.processId);
  }
}
