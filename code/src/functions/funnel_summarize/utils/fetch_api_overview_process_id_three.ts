...
export class FetchFunnelAPIOverviewPartThreeSkill implements Skill {
  
  // devrevSDK is used to make API calls using service account token.
  devrevSDK: SdkUtils;
  // Name of the skill.
  Name = 'FetchFunnelAPIOverviewPartThreeSkill';
  ...
  async Execute(skillCall: SkillCall): Promise<any> {
    ...
    return this.devrevSDK.getFunnelOverviewPartThree(skillCall.args.id, skillCall.args.appId, skillCall.args.appVersion, skillCall.args.platform, skillCall.args.fromDate, skillCall.args.toDate, skillCall.args.processId);
  }
}
